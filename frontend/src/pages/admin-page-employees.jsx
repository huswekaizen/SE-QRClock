...
import DeleteEmployeeModal from "../components/delete-employee-modal.jsx";
...
export default function AdminPageEmployees() {
    ...
    const handleDeleteEmployee = async (id, setError) => {
        try {
            ...
            if (!response.ok) {
                if (setError) setError(result.error || "Failed to delete employee");
                return false;
            }

        } catch (err) {
            console.error(err);
            if (setError) setError("Failed to delete employee");
            return false;
        }
    };
